...
const PosManagerDashboard = ({ shiftId }) => {
  ...
  const fetchSalesSummary = useCallback(async () => {
    try {
      ...
      // Only add date filters if they are not the default (today)
  ...
  const toggleRowExpansion = (shiftId) => {
    ...
    // Reset sale expansion and shift tab when collapsing shift
  ...
  const handleVoidConfirm = async () => {
    ...
    try {
      ...
      // After voiding, refresh the data
  ...
  const handleEditConfirm = async () => {
    try {
      // The EditTransactionModal handles the API call
      await fetchSalesSummary(); // Refresh data after edit
                                 ...
                                 <div className="shift-detail-tabs">
                                   ...
                                   {shiftTab === 'sales' && shift.sales && shift.sales.length > 0 && (
                                     <div className="shift-sales-detail">
                                       <table className="shift-sales-table">
                                         <thead>
                                           <tr>
                                             <th>Time</th>
                                             <th>Receipt #</th>
                                           ...
                                           {shift.sales
                                             ...
                                             <React.Fragment key={sale.id || saleIndex}>
                                               ...
                                               {expandedSale === sale.id && sale.items && sale.items.length > 0 && (
                                                 <tr className="expanded-sale-row">
                                                   <td colSpan="8" className="expanded-sale-content">
                                                     <div className="sale-items-detail">
                                                       <h6>Items Sold ({sale.items.length})</h6>
                                         ...
                                         <tbody>
                                           {shift.voided_sales.map((sale, saleIndex) => (
                                             <tr key={sale.id || saleIndex}>
                                               ...
                                               <td className="customer-cell">{sale.customer}</td>
                                               <td className="amount-cell">{formatCurrency(sale.total_amount)}</td>
                                               <td className="void-reason-cell">{sale.void_reason || 'N/A'}</td>
                                               <td className="date-cell">
                                                 {sale.voided_at ? new Date(sale.voided_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : 'N/A'}
                                               </td>
                                             </tr>
                                           ))}
                                         </tbody>
